...
DataBase <- src_sqlite("my_db.sqlite1", create = T)
...
p <- ggplot(flights_canceled_month, aes(x= factor(month),y=canceled)) + geom_bar(stat="identity")
p
...
p <- ggplot(flights_canceled_day, aes(x= factor(day),y=canceled)) + geom_bar(stat="identity")
p
```
